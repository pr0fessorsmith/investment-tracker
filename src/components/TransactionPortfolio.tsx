    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {portfolio.positions.map((position) => (
          <div key={position.symbol} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="text-xl font-bold text-gray-900 dark:text-white">{position.symbol}</h4>
                  <p className="text-gray-600 dark:text-gray-400">
                    {position.totalShares.toFixed(5).replace(/\.?0+$/, '')} shares @ {formatCurrency(position.averageCostPerShare)} avg cost
                  </p>
                </div>
                <button
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <p className={`font-semibold ${getGainLossColor(position.realizedGainLoss)}`}>
                    {formatCurrency(position.realizedGainLoss)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Unrealized P&L</p>
                  <p className={`font-semibold ${getGainLossColor(position.unrealizedGainLoss || 0)}`}>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {transaction.notes && (
                                <p className="text-sm text-gray-600 dark:text-gray-300 italic">{transaction.notes}</p>
                              )}
                            </div>
                          </div>
